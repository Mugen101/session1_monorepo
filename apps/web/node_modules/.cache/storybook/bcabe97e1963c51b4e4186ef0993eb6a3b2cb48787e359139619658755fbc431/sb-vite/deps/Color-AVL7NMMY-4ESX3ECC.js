...
import "./chunk-S3JGQ73N.js";
import "./chunk-57ERTNE6.js";
import "./chunk-ILZF3T27.js";
import "./chunk-XBBVD6EC.js";
import "./chunk-R4JTZDSR.js";
import "./chunk-NSHMDNEI.js";
import {
  require_react
...
var PickerTooltip = xr(Y7)({ position: "absolute", zIndex: 1, top: 4, left: 4, "[aria-readonly=true] &": { cursor: "not-allowed" } });
...
var Note = xr(i6)(({ theme }) => ({ fontFamily: theme.typography.fonts.base }));
...
var Input = xr(Z3.Input)(({ theme, readOnly }) => ({ width: "100%", paddingLeft: 30, paddingRight: 30, boxSizing: "border-box", fontFamily: theme.typography.fonts.base }));
...
var ColorControl = ({ name, value: initialValue, onChange, onFocus, onBlur, presetColors, startOpen = false, argType }) => {
  let debouncedOnChange = (0, import_react.useCallback)(debounce2(onChange, 200), [onChange]), { value, realValue, updateValue, color, colorSpace, cycleColorSpace } = useColorInput(initialValue, debouncedOnChange), { presets, addPreset } = usePresets(presetColors ?? [], color, colorSpace), Picker = ColorPicker[colorSpace], readonly = !!argType?.table?.readonly;
  return import_react.default.createElement(Wrapper, { "aria-readonly": readonly }, import_react.default.createElement(PickerTooltip, { startOpen, trigger: readonly ? null : void 0, closeOnOutsideClick: true, onVisibleChange: () => color && addPreset(color), tooltip: import_react.default.createElement(TooltipContent, null, import_react.default.createElement(Picker, { color: realValue === "transparent" ? "#000000" : realValue, onChange: updateValue, onFocus, onBlur }), presets.length > 0 && import_react.default.createElement(Swatches, null, presets.map((preset, index) => import_react.default.createElement(Y7, { key: `${preset?.value || index}-${index}`, hasChrome: false, tooltip: import_react.default.createElement(Note, { note: preset?.keyword || preset?.value || "" }) }, import_react.default.createElement(Swatch, { value: preset?.[colorSpace] || "", active: !!(color && preset && preset[colorSpace] && id(preset[colorSpace] || "") === id(color[colorSpace])), onClick: () => preset && updateValue(preset.value || "") }))))) }, import_react.default.createElement(Swatch, { value: realValue, style: { margin: 4 } })), import_react.default.createElement(Input, { id: getControlId(name), value, onChange: (e2) => updateValue(e2.target.value), onFocus: (e2) => e2.target.select(), readOnly: readonly, placeholder: "Choose color..." }), value ? import_react.default.createElement(ToggleIcon, { onClick: cycleColorSpace }) : null);
};
...
//# sourceMappingURL=Color-AVL7NMMY-4ESX3ECC.js.map
