...
(() => {
  ...
  // ../../node_modules/@storybook/addon-onboarding/dist/Onboarding-64RGZKLB.js
  var Onboarding_64RGZKLB_exports = {};
  __export(Onboarding_64RGZKLB_exports, {
    default: () => Onboarding
  });
  function y(y2, z2 = {}) {
  ...
  function Onboarding({ api }) {
    ...
    }, [api, setEnabled]), completeOnboarding = useCallback((answers) => {
      api.emit(STORYBOOK_ADDON_ONBOARDING_CHANNEL, { step: "7:FinishedOnboarding", type: "telemetry" }), api.emit(STORYBOOK_ADDON_ONBOARDING_CHANNEL, { answers, type: "survey" }), selectStory("configure-your-project--docs"), disableOnboarding();
...
`, Tooltip = ({ index, size, step, closeProps, primaryProps, tooltipProps }) => (useEffect(() => {
        ...
        let [isSubmitting, setIsSubmitting] = useState(!1), [formFields, setFormFields] = useState({ building: { label: "What are you building?", type: "checkbox", required: !0, options: shuffleObject({ "design-system": { label: "Design system" }, "application-ui": { label: "Application UI" } }), values: { "design-system": !1, "application-ui": !1 } }, interest: { label: "Which of these are you interested in?", type: "checkbox", required: !0, options: shuffleObject({ "ui-documentation": { label: "Generating UI docs" }, "functional-testing": { label: "Functional testing" }, "accessibility-testing": { label: "Accessibility testing" }, "visual-testing": { label: "Visual testing" }, "ai-augmented-development": { label: "Building UI with AI" }, "team-collaboration": { label: "Team collaboration" }, "design-handoff": { label: "Design handoff" } }), values: { "ui-documentation": !1, "functional-testing": !1, "accessibility-testing": !1, "visual-testing": !1, "ai-augmented-development": !1, "team-collaboration": !1, "design-handoff": !1 } }, referrer: { label: "How did you discover Storybook?", type: "select", required: !0, options: shuffleObject({ "we-use-it-at-work": { label: "We use it at work" }, "via-friend-or-colleague": { label: "Via friend or colleague" }, "via-social-media": { label: "Via social media" }, youtube: { label: "YouTube" }, "web-search": { label: "Web Search" }, "ai-agent": { label: "AI Agent (e.g. ChatGPT)" } }), values: { "we-use-it-at-work": !1, "via-friend-or-colleague": !1, "via-social-media": !1, youtube: !1, "web-search": !1, "ai-agent": !1 } } }), updateFormData = (key, optionOrValue, value) => {
          ...
          setFormFields((fields) => {
            ...
            if (field.type === "select") {
              let values = Object.fromEntries(Object.entries(field.values).map(([opt]) => [opt, opt === optionOrValue]));
              return { ...fields, [key]: { ...field, values } };
            }
            return fields;
          });
        }, isValid = Object.values(formFields).every((field) => field.required ? Object.values(field.values).some((value) => value === !0) : !0);
        return react_default.createElement(Modal, { defaultOpen: !0, width: 420, onEscapeKeyDown: onDismiss }, react_default.createElement(Form, { onSubmit: (e2) => {
          isValid && (e2.preventDefault(), setIsSubmitting(!0), onComplete(Object.fromEntries(Object.entries(formFields).map(([key, field]) => [key, field.values]))));
        }, id: "intent-survey-form" }, react_default.createElement(Content, null, react_default.createElement(Modal.Header, null, react_default.createElement(Modal.Title, null, "Help improve Storybook")), Object.keys(formFields).map((key) => {
          let field = formFields[key];
          return react_default.createElement(react_default.Fragment, { key }, react_default.createElement(Question, null, field.label), field.type === "checkbox" && react_default.createElement(Row, null, Object.entries(field.options).map(([opt, option]) => {
            let id = `${key}:${opt}`;
            return react_default.createElement("div", { key: id }, react_default.createElement(Label, { htmlFor: id }, react_default.createElement(Checkbox, { name: id, id, checked: field.values[opt], disabled: isSubmitting, onChange: (e2) => updateFormData(key, opt, e2.target.checked) }), option.label));
          })), field.type === "select" && react_default.createElement(Form.Select, { name: key, id: key, value: Object.entries(field.values).find(([, isSelected]) => isSelected)?.[0] || "", required: field.required, disabled: isSubmitting, onChange: (e2) => updateFormData(key, e2.target.value) }, react_default.createElement("option", { disabled: !0, hidden: !0, value: "" }, "Select an option..."), Object.entries(field.options).map(([opt, option]) => react_default.createElement("option", { key: opt, value: opt }, option.label))));
        }), react_default.createElement(Actions, null, react_default.createElement(Button, { disabled: isSubmitting || !isValid, size: "medium", type: "submit", variant: "solid" }, "Submit")))));
      };
  ...
  // node_modules/.cache/storybook/bcabe97e1963c51b4e4186ef0993eb6a3b2cb48787e359139619658755fbc431/sb-manager/onboarding-3/manager-bundle.js
  init_define_module();
  init_define_process_env();
  init_define_process_env_NODE_PATH();
  ...
  // ../../node_modules/@storybook/addon-onboarding/dist/manager.js
  var Onboarding2 = lazy(() => Promise.resolve().then(() => (init_Onboarding_64RGZKLB(), Onboarding_64RGZKLB_exports)));
